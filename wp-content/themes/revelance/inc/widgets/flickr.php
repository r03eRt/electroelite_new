<?php
class ABdev_dzen_flickr_stream extends WP_Widget {
	
	function ABdev_dzen_flickr_stream(){
		$widget_ops = array(
			'classname' => 'flickr-stream', 
			'description' => __('Photostream from Flickr.', 'ABdev_dzen'),
		);
		$control_ops = array(
			'id_base' => 'flickr-stream',
		);
		$this->WP_Widget('flickr-stream', __('Flickr stream', 'ABdev_dzen' ), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance){
		...
		if($flickr_username && $number) {
			...
			if(is_object($stream) && isset($stream->user->id)) {
				...
				echo '<p>'.__('Invalid flickr username or connection error.', 'ABdev_dzen').'</p>';
			}
		}
		echo $after_widget;
	}
	...
	function form($instance){
		$defaults = array('title' => __('Photos from Flickr', 'ABdev_dzen'), 'flickr_username' => '', 'number' => 9);
		$instance = wp_parse_args((array) $instance, $defaults); ?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'ABdev_dzen');?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('flickr_username'); ?>"><?php _e('Flickr username:', 'ABdev_dzen');?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('No. of images:', 'ABdev_dzen');?></label>
...
function ABdev_dzen_flickr_stream_widget(){
	register_widget('ABdev_dzen_flickr_stream');
}

add_action('widgets_init', 'ABdev_dzen_flickr_stream_widget');